...
export function OutfitGenerator({ onBack }: OutfitGeneratorProps) {
  ...
  const [selectedItemToSwap, setSelectedItemToSwap] = useState<OutfitItem | null>(null);
  ...
  const handleLaundryBasket = (item: OutfitItem) => {
    ...
    if (alternatives.length > 0) {
      ...
      setOutfit(newOutfit);
      toast.success(`🧺 ${item.name} marked as in laundry. Auto-selected ${randomAlt.name}!`);
    }
  };
          ...
          <Card className="border-2 border-purple-100 w-full max-w-sm">
            <CardHeader className="text-center">
              ...
              <CardDescription className="text-base mt-2">
                Let AI create your perfect outfit based on today's weather and your style preferences
              </CardDescription>
            </CardHeader>
  ...
  return (
    <div className="flex flex-col pb-20">
      ...
      <div className="p-4 space-y-4">
        ...
        <Card className="border-2 border-purple-100 bg-gradient-to-br from-purple-50 to-white">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-gray-600">This outfit perfectly matches your preferences and the weather!</p>
          </CardContent>
        </Card>
        ...
        <div className="space-y-3">
          {outfit.map((item) => (
            <Card key={item.id} className="border-2 border-purple-100 overflow-hidden">
              <CardContent className="p-4">
      ...
      {/* Swap Item Dialog */}
      <Dialog open={!!selectedItemToSwap} onOpenChange={() => setSelectedItemToSwap(null)}>
          ...
          {selectedItemToSwap && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(alternativeItems[selectedItemToSwap.category] || []).map((altItem) => (
                  <button
                    key={altItem.id}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                      <div className="absolute bottom-2 left-2 right-2">
                        <p className="text-white text-xs truncate">{altItem.name}</p>
                      </div>
                    </div>
